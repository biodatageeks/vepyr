...
- A2 separately magnifies CADD's `manifest.json`: `cache_source_version` pins the requested ref and resolved commit; `sources[]` records source parts, upstream URLs, declared MD5 and the actual digest when verified, plus input/index fingerprints. This JSON sidecar is distinct from the row-level provenance columns of core transcript/exon/regulatory/motif entities. See `docs/plugins.md`, `tests/test_build_plugin_cache.py` and `src/vepyr/__init__.py`. Angle-bracket values illustrate the schema, not a measured CADD build.
