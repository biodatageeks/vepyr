...
def annotate(
    ...
    *,
    ...
    forks: int = 0,
    skip_csq: bool = True,
    warm_variation_cache: bool = False,
    warm_variation_dir: str | None = None,
    variation_cold_dir: str | None = None,
    variation_position_index_dir: str | None = None,
    warm_variation_batch_size: int | None = None,
    # Output mode
    ...
    warm_variation_dir : str or None
        ...
        ``cache_dir/variation``.
    variation_cold_dir : str or None
        Directory containing ``<chrom>_cold.parquet`` files. Defaults to
        ``cache_dir/variation``.
    variation_position_index_dir : str or None
        Directory containing ``<chrom>.posidx`` cold-position indexes.
        Defaults to ``cache_dir/variation.position_index``.
    warm_variation_batch_size : int or None
        Parquet batch size for loading warm row groups. ``None`` uses the
        engine default.
    skip_csq : bool
    ...
    if forks > 0 and not use_fjall:
        raise ValueError("forks > 0 requires use_fjall=True")
    if warm_variation_cache and not use_fjall:
        raise ValueError("warm_variation_cache=True requires use_fjall=True")
    if warm_variation_batch_size is not None and (
        isinstance(warm_variation_batch_size, bool)
        or not isinstance(warm_variation_batch_size, int)
        or warm_variation_batch_size <= 0
    ):
        raise ValueError("warm_variation_batch_size must be a positive integer")
    ...
    opts: dict = {
        ...
        "forks": forks,
        "inline_lookup": forks == 0,
    }

    if use_fjall:
        opts["use_fjall"] = True
    if warm_variation_cache:
        opts["warm_variation_cache"] = True
    if warm_variation_dir is not None:
        opts["warm_variation_dir"] = warm_variation_dir
    if variation_cold_dir is not None:
        opts["variation_cold_dir"] = variation_cold_dir
    if variation_position_index_dir is not None:
        opts["variation_position_index_dir"] = variation_position_index_dir
    if warm_variation_batch_size is not None:
        opts["warm_variation_batch_size"] = warm_variation_batch_size
