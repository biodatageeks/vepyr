...
fn filter_requested_chromosomes(
    ...
    let requested = requested.map(|values| {
        values
            .into_iter()
            .map(|value| value.strip_prefix("chr").unwrap_or(&value).to_string())
            .collect::<HashSet<_>>()
    });

    match (discovered, requested) {
        (Some(values), Some(requested)) => Some(
            values
                .into_iter()
                .filter(|value| requested.contains(value.strip_prefix("chr").unwrap_or(value)))
                .collect(),
        ),
        (None, Some(requested)) => Some(requested.into_iter().collect()),
        (values, None) => values,
    }
}
...
async fn convert_entity_per_chrom(
    ...
    let chroms = filter_requested_chromosomes(chroms_from_schema(&provider_schema), chromosomes);
    drop(init_ctx);
