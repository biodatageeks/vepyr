...
`performance-tests/vepyr/scripts/lf_run_pool_bench.py`:
...
def child(mode: str, workers: int, src: str, plugins: str, args: argparse.Namespace) -> None:
    import vepyr
...
def run_once(cmd: list[str], env: dict[str, str]) -> dict:
    ...
    return {**json.loads(stdout.strip().splitlines()[-1]), "maxrss_gib": rss_bytes / 2**30}
...
def main() -> None:
    ...
    p.add_argument("--modes", nargs="+", choices=["raw", "lf", "vcf"], default=["raw", "lf", "vcf"])
    p.add_argument("--plugin-sets", nargs="+", choices=["none", "all"], default=["none", "all"])
    ...
    base = [sys.executable, os.path.abspath(__file__), "--out-dir", str(out_dir),
            "--input", "x=x", "--cache-dir", args.cache_dir, "--fasta", args.fasta,
            "--plugin-cache-root", args.plugin_cache_root]
    rows = []
            ...
            for mode in args.modes:
                for w in args.workers:
                    ...
                    for i in range(1 + args.repeats):
                        ...
                        r = run_once(base + ["--child", mode, str(w), src, plugins], env)
                        r["load_before"] = round(load, 2)
                        runs.append(r)
                    kept = runs[1:]
                    row = {
                        "input": name, "plugins": plugins, "mode": mode, "workers": w,
                        "env": args.env,
                        "median_wall_s": round(statistics.median(r["wall_s"] for r in kept), 3),
                        "min_wall_s": round(min(r["wall_s"] for r in kept), 3),
                        "max_wall_s": round(max(r["wall_s"] for r in kept), 3),
                        "median_rss_gib": round(statistics.median(r["maxrss_gib"] for r in kept), 2),
                        "median_engine_wait_s": round(statistics.median(r["engine_wait_s"] for r in kept), 3),
                        "median_consumer_s": round(statistics.median(r["consumer_s"] for r in kept), 3),
...
def render_summary(rows: list[dict]) -> str:
    ...
    for (name, plugins) in sorted({(r["input"], r["plugins"]) for r in rows}):
        top = max(r["workers"] for r in rows if (r["input"], r["plugins"]) == (name, plugins))
        pick = {r["mode"]: r["median_wall_s"] for r in rows
                if (r["input"], r["plugins"], r["workers"]) == (name, plugins, top)}
        if "lf" in pick and "vcf" in pick:
            lines.append(f"- {name} plugins={plugins} w{top}: lf/vcf = {pick['lf'] / pick['vcf']:.2f}")
    return "\n".join(lines) + "\n"
...
`run_once` reaps the child with `os.wait4` so `ru_maxrss` is that child's own peak, and it reads the output through temporary files so a full pipe cannot deadlock the wait.
...
uv run python "$ROOT/tools/vepyr-fix/compare_runs.py" "$BASE/archive" "$FINAL/archive" || echo "WGS perf gate FAILED"
