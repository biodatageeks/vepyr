...
- Executed inline in the worktree `~/research/git/_wt/dbfn-stream-pool` (branch `feat/stream-run-pool` from `dba94e3`); engine PR biodatageeks/datafusion-bio-functions#240, head `0c08e2a`.
- Deviation from Task 4: a run's lookup plan inherits the VCF scan's partition count (vepyr's session sets `target_partitions = workers`), so `activate_run_lookup` returned one handle per partition and the pool's "exactly one handle" check fired. Fix (commit `0c08e2a`): in `activate_run_lookup`, a pool run (`annotation_workers > 1` and no shard context) reads every partition of its filtered plan in id order through `spawn_lookup_full_contig_worker`, the worker the sink's grid slices already use, so the run task still consumes one ordered channel. `config.target_partitions = 1` from Task 2 is kept (it only bounds the probe parallelism inside a run).
- Task 1's `plan_stream_runs_keeps_region_bounds_and_warm_up` expected overlap 150 to reach back one buffer; `build_grid_slices` walks back to the first buffer at or below `seam - overlap`, so 150 from seam 200 reaches buffer 0. The test uses overlap 100.
- Task 4 also needed the pool arm in `cleanup_registered_tables_on_drop` (the compiler's exhaustiveness check caught it); `RunOutput::Shard` boxes the writer for clippy's `large_enum_variant`.
- Task 7 smoke: all eight checks `equal=True` with one-buffer runs and with the default run length; the trace showed 15 runs planned and released for the fixture with three in flight. vepyr's sink parity test skipped (raw cache fixture absent).
