...
def discover_rows(
    input_dir: Path, cache_type: str, *, fractional: bool = False
) -> list[RunRow]:
    rows: list[RunRow] = []
    for time_file in sorted(
        input_dir.glob(f"{cache_type}_fork*.time.txt"),
        key=lambda item: fork_sort_key(fork_from_path(item)),
    ):
        ...
        status = (
            "OK"
            if elapsed_wall and exit_status in {"", "0"}
        ...
        rows.append(
            RunRow(
                ...
                warnings_file=input_dir
                / f"HG002_annotated_wgs_everything_hgvs_{cache_type}_fork{fork}.vcf_warnings.txt",
            )
        )
    return rows
        ...
        for row in sorted(rows, key=lambda item: fork_sort_key(item.fork)):
            writer.writerow(
                [
                    ...
                    if float(row.elapsed_seconds).is_integer()
...
def command_text(cache_type: str, forks: list[str], release: str = "116") -> str:
    ...
    ordered_forks = sorted(
        (fork for fork in forks if fork != "none"), key=int, reverse=True
    )
    if "none" in forks:
...
def plot_rows(rows: list[RunRow], args: argparse.Namespace) -> None:
    ...
    fig.text(
        ...
        0.965,
        (
            args.command_file.read_text()
            if args.command_file
            else command_text(args.cache_type, labels, args.release)
        ).replace("$", r"\$"),
        ha="left",
        va="top",
        family="monospace",
...
def plot_scaling_panels(rows: list[RunRow], args: argparse.Namespace) -> None:
    ...
    bars = runtime_ax.bar(
        positions,
        times,
        color=["0.8" if row.fork == "none" else "0.3" for row in rows],
        edgecolor="black",
        linewidth=0.7,
        width=0.62,
    )
    ...
    speedup_ax.plot(
        positions, speedups, "o-", color="black", linewidth=1.6, markersize=6
    )
    ...
    speedup_ax.set_ylabel(
        f"Speedup relative to {'no --fork' if baseline.fork == 'none' else '--fork ' + baseline.fork}"
    )
    ...
    if args.command_file:
        ...
        invocation = command_text(
            args.cache_type, [row.fork for row in rows], args.release
        )
    fig.text(
        0.07,
        0.365,
...
def main() -> None:
    ...
    parser.add_argument(
        "--command-file",
        type=Path,
        help="Embed the exact saved invocation instead of a generic example",
    )
    parser.add_argument(
        "--scaling-panels",
        action="store_true",
        help="Monochrome wall-time and speedup panels; exports PNG/PDF/SVG",
    )
    parser.add_argument("--environment", default="")
    parser.add_argument("--dataset", default="HG002 GRCh38")
    parser.add_argument(
        "--run-note", default="One run per setting; exploratory benchmark."
    )
    args = parser.parse_args()

    rows = discover_rows(
        args.input_dir, args.cache_type, fractional=args.scaling_panels
    )
